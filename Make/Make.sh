...
#---------------------------------------------------------------------------------------------------
#Script Description- <name>https://drive.google.com/drive/u/1/my-drive
#Program Description:
#	<info>
#Post Triggers
#	<info>
...
function exec_make()
{
    ...
    echo $DDASH
    pt "                           Single Make Name:$program_name               "
    pt "                           Single Make Type $program_type"
    f_$1
    
    pt "                            Single Make demmand completed "
    echo $DDASH
    echo
    echo
}
...
function f_eclipse()
{
    make_clean()
    {
        ${BASH_CMD} "${SNAP_REMOVE} $ECLIPSE"
    }
    make_build()
    {
     ${BASH_CMD} "${SNAP_INSTALL} $ECLIPSE ${CLASSIC_FLAG}"
...
function f_eclipse_old()
{
ECLIPSE_NAME='eclipse'
ECLIPSE_URL='http://www.mirrorservice.org/sites/make_download.eclipse.org/eclipseMirror/technology/epp/make_downloads/release/2019-12/M2/eclipse-java-2019-12-M2-linux-gtk-x86_64.tar.gz'
...
LIBREOFFICE_URL='https://download.documentfoundation.org/libreoffice/stable/6.4.0/deb/x86_64/LibreOffice_6.4.0_Linux_x86-64_deb.tar.gz'
...
{
    VIDEO_NAME='totem'
    VIDEO_264_PLUGIN='gstreamer1.0-plugins-ugly'
    make_build() {
        ${BASH_CMD} "${APT_INSTALL} ${VIDEO_NAME}"
        ${BASH_CMD} "${APT_INSTALL} ${VIDEO_264_PLUGIN}"

    }
    make_clean() {
        ${BASH_CMD} "${APT_PURGE} ${VIDEO_NAME}"
                ${BASH_CMD} "${APT_PURGE} ${VIDEO_264_PLUGIN}"
...
#	<info>
#--------------------------------------------------------------------------------------------------
declare readonly OPERA='opera'
function f_video()
{
    make_build() {
        ${BASH_CMD} "${SNAP_INSTALL} ${OPERA}"
        ${BASH_CMD} "${SNAP_INSTALL} ${OPERA}"

    }
    make_clean() {
        ${BASH_CMD} "${SNAP_REMOVE} ${OPERA}"
    }
    make_target() {
        make_clean
        make_build
...
function f_admin()
{
    ...
    PROGRAM_BACKUP_DIR=${PROGRAM_SRC_DIR}${BACKUP}${SLASH}
    ...
    BACKUP_FILE_PATH=${PROGRAM_BACKUP_DIR}${PROGRAM_NAME}${EPOCH_TIME}${BKUP_EXT}
    
    
    make_version() 
    {
        
        ${BASH_CMD} "${MK_DIR} ${PROGRAM_BACKUP_DIR}"
        ${BASH_CMD} "${FORCE_CP} ${PROGRAM_SRC_PATH} ${BACKUP_FILE_PATH}"
        ${BASH_CMD} "${CHMOD_NONRUNNABLE} ${BACKUP_FILE_PATH}"
        ${BASH_CMD} "${CHMOD_READONLY} ${BACKUP_FILE_PATH}"
    ...
    make_deploy() {
        ...
        ${BASH_CMD} "${CHMOD_READONLY} ${PROGRAM_NAME}"Make
...
parse_parameters() {
    ...
    if [ "$parameter1" == "" ]; then
        f_help
        elif [ "$parameter1" == "reveal" ]; then 
            reveal $parameter2 $parameter3
            exit 0
...
echo "Error Log For Make" > 'MakeError.log'
echo "            Date : $(date)"
echo "$STARS"
parameter1=$1 parameter2=$2 parameter3=$3
parse_parameters
if [ "$command_type" == "$BASH_CMD" ] ; then
             ${BASH_CMD} "$parameter1 $parameter2 $parameter3"
        elif [ "$command_type" == "${EXEC_MAKE}" ] ; then
             ${EXEC_MAKE} "${program_name}"
        elif [ "$command_type" == "$EXEC_BUILD" ] ; then
        echo " ${STARS}"
             ${EXEC_BUILD} "${program_name}"
        echo "${STARS}"
        
    else
        pt "Need to run parse parameeters to direct parameters correctly."
